...
async function generateInventoryCard(userDisplayName, categoryTitle, items, page, totalPages, selectedIndex = 0) {
    ...
    for (let i = 0; i < 15; i++) {
        ...
        if (isSelected) {
            ...
            roundRect(ctx, x - 10, y - 10, slotSize + 20, slotSize + 20, 25);
            ctx.fill();
            ctx.shadowBlur = 30;
            ctx.fillStyle = 'rgba(0, 255, 255, 0.3)';
            roundRect(ctx, x + 10, y + 10, slotSize - 20, slotSize - 20, 15);
            ctx.fill();
            ctx.restore();
        }
        ...
        if (isSelected) {
            ...
            const x_l = x - offset;
            const y_u = y - offset;
            const x_r = x + slotSize + offset;
            const y_d = y + slotSize + offset;

            ctx.beginPath();
            ctx.moveTo(x_l, y_u + bracketLen); ctx.lineTo(x_l, y_u); ctx.lineTo(x_l + bracketLen, y_u);
            ctx.moveTo(x_r - bracketLen, y_u); ctx.lineTo(x_r, y_u); ctx.lineTo(x_r, y_u + bracketLen);
            ctx.moveTo(x_r, y_d - bracketLen); ctx.lineTo(x_r, y_d); ctx.lineTo(x_r - bracketLen, y_d);
            ctx.moveTo(x_l + bracketLen, y_d); ctx.lineTo(x_l, y_d); ctx.lineTo(x_l, y_d - bracketLen);
            ctx.stroke();
            ctx.restore();
        }
    }
...
// ========================================================
// 🔥 دالة 3: تصميم صفحة تفاصيل العنصر (الاحترافية القصوى) 🔥
// ========================================================
async function generateItemDetailsCard(userDisplayName, item) {
    ...
    // 2. إضافة إضاءة خلفية بلون ندرة العنصر
    ...
    // 4. رسم صورة العنصر (كبيرة في اليسار)
    const imgSize = 350;
    const imgX = 100;
    ...
    // 5. كتابة تفاصيل العنصر (في اليمين)
    const textX = width - 80; // الكتابة من اليمين لليسار
    let textY = 120;
    ...
    // وصف افتراضي إذا لم يوجد
    const description = item.description || "عنصر غامض لا يُعرف عنه الكثير. ربما يكون مفيداً في وقت لاحق، أو ربما يكون مجرد خردة تملأ حقيبتك.";
