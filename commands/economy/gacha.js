...
// جلب المولد والإعدادات
const { generateGachaCard } = require('../../generators/gacha-generator.js');
...
upgradeMats.weapon_materials.forEach(race => {
    race.materials.forEach(m => {
        const raceFolder = race.race.toLowerCase();
        // التسمية المختصرة التي اتفقنا عليها: عرق_اسم (مثلاً: dragon_ash)
        const imgName = m.id.replace('mat_', ''); 
        LOOT_POOL[m.rarity].push({ 
            ...m, 
            type: 'material', 
            race: race.race,
            imgPath: `images/materials/${raceFolder}/${imgName}.png`
        });
    });
});
...
upgradeMats.skill_books.forEach(cat => {
    cat.books.forEach(b => {
        ...
        LOOT_POOL[b.rarity].push({ 
            ...
            imgPath: `images/skill_books/${typeFolder}/${b.id}.png`
        });
    });
});

// إضافة المهارات (كمهارات نصية حالياً أو أيقونات عامة)
...
module.exports = {
    ...
    async execute(interactionOrMessage) {
        ...
        collector.on('collect', async (i) => {
            ...
            // 🎨 توليد الصورة (للسحبة الواحدة أو للأفضل في العشر)
            let files = [];
            const finalEmbed = new EmbedBuilder()
                .setTitle(isTen ? '🌟 نتائج السحب الملكي' : '📦 غنيمة الصندوق')
                .setFooter({ text: `الضمان: الملحمي (${10 - pityData.epic_pity}) | الأسطوري (${90 - pityData.legendary_pity})` });

            if (bestResult && bestResult.item.imgPath) {
            ...
            await msg.edit({ embeds: [finalEmbed], components: [buildUI().components[0]], files: files });
        });
    }
};
