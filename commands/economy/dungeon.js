    ...
    async execute(context, args) {
        ...
        if (isSlash) {
            ...
            interaction = {
                ...
                deferReply: async () => {},
                deferUpdate: async () => {},
                // محاكاة isButton لمنع الكراش في أماكن أخرى (احتياط)
                isButton: () => false 
            };
        }
        ...
        if (user.id !== OWNER_ID) {
            ...
            // إذا كان عليه كولداون
            if (diff < COOLDOWN_MS) {
                // التأكد من التذاكر (للانضمام)
        ...
        // 4. تشغيل النظام فوراً (تم حذف كود الأسئلة من هنا)
        // سيقوم الهاندلر (dungeon-handler.js) بفحص الحفظ تلقائياً وبدء اللوبي
        try {
