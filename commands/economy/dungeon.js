...
async function lobbyPhase(interaction, oldMsg, theme, sql) {
    ...
    collector.on('end', async (c, reason) => {
        if (reason === 'start') {
            ...
            // 🔥 حالة الإلغاء (انتهاء الوقت أو إلغاء القائد) 🔥
            activeDungeonRequests.delete(host.id);
            if (msg.editable) {
                ...
                if (oldEmbed) {
                    const cancelledEmbed = EmbedBuilder.from(oldEmbed)
                        ...
                        .setColor('Red') // تغيير اللون للأحمر
                        .setFooter({ text: "انتــهى الوقــت" });
                    
                    // الاحتفاظ بالإيمبد وإزالة الأزرار
