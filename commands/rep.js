...
// آي دي المالك (أنت) لتخطي القوانين
const OWNER_ID = "1145327691772481577";

// دالة مساعدة لمعرفة التصنيف بناءً على النقاط الجديدة
...
// دالة لحساب موعد التصفير القادم بتوقيت السعودية
function getNextResetTime() {
    ...
    const resetDate = new Date(rsaDateString + ' 23:59:59 GMT+0300'); // نهاية اليوم في السعودية
...
module.exports = {
    ...
    usage: '-rep <@user>',
    // 🔥 تم إضافة كل الاختصارات الممكنة هنا 🔥
    ...
    async execute(message, args) {
        ...
        // 2. التحقق من مستوى (Level) المرسل (يجب أن يكون 10 أو أعلى) - مع استثناء المالك
        ...
        // 3. إعداد سجلات السمعة في الداتابيس (إذا لم تكن موجودة)
        ...
        // 4. التحقق من الكولداون بناءً على تاريخ اليوم (بتوقيت مكة) - مع استثناء المالك
        ...
        // 5. تنفيذ العملية (إضافة النقطة وتحديث وقت المرسل)
        const newTargetPoints = targetRep.rep_points + 1;
        
        sql.transaction(() => {
            // تحديث المستلِم (زيادة النقاط)
        ...
        // 6. جلب بيانات التصنيف الجديد للمستلِم وإنشاء الصورة
        ...
        // إرسال رسالة "جاري التحميل" لأن رسم الصورة يأخذ جزء من الثانية
        ...
        try {
            ...
            // رسالة نصية قصيرة مع الصورة
            let msgContent = `🌟 **لقد قام <@${senderId}> بتزكية <@${targetId}>!**`;
            if (isRankUp) msgContent += `\n🎉 **تهانينا! لقد ارتقيت إلى ${targetRankData.name}!**`;

            await message.reply({ content: msgContent, files: [attachment] });
        } catch (error) {
