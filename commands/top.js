...
async function fetchLeaderboardData(client, sql, guild, type, page, targetUserId = null) {
    ...
    try {
        if (type === 'level') {
            ...
            // 🔥 الإصلاح: استدعاء المجموع كنص (TEXT) مباشرة من السحابة لضمان الدقة المطلقة
        ...
        let totalMora = null;
        if (type === 'mora') {
            // 🔥 الإصلاح: جلب المجموع الكلي للسيرفر كنص لضمان عدم ظهور أرقام فلكية في أسفل الصورة
        ...
        const enrichedData = await Promise.all(pageDataRaw.map(async (u) => {
            ...
            return {
                uid: uid,
                db: u, // سيحتوي على total_wealth كنص صحيح للأثرياء
