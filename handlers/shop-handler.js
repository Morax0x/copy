...
async function _handleRodUpgrade(i, client, db) {
    ...
    let userDataRes;
    try { userDataRes = await db.query(`SELECT "mora", "bank" FROM levels WHERE "user" = $1 AND "guild" = $2`, [i.user.id, i.guild.id]); }
    catch(e) { userDataRes = await db.query(`SELECT mora, bank FROM levels WHERE userid = $1 AND guildid = $2`, [i.user.id, i.guild.id]).catch(()=>({rows:[]})); }
    
    let fishDataRes;
    try { fishDataRes = await db.query(`SELECT * FROM user_fishing WHERE "userID" = $1 AND "guildID" = $2`, [i.user.id, i.guild.id]); }
    catch(e) { fishDataRes = await db.query(`SELECT * FROM user_fishing WHERE userid = $1 AND guildid = $2`, [i.user.id, i.guild.id]).catch(()=>({rows:[]})); }
    ...
    try {
        await db.query(`UPDATE levels SET "mora" = "mora" - $1 WHERE "user" = $2 AND "guild" = $3`, [nextRod.price, i.user.id, i.guild.id]);
        moraSuccess = true;
    } catch(e) {
        try {
            await db.query(`UPDATE levels SET mora = mora - $1 WHERE userid = $2 AND guildid = $3`, [nextRod.price, i.user.id, i.guild.id]);
            moraSuccess = true;
        } catch(e2) {}
    }
    ...
    try {
        if (Object.keys(fishRow).length > 0) {
            try {
                await db.query(`UPDATE user_fishing SET "rodLevel" = $1, "currentRod" = $2 WHERE "userID" = $3 AND "guildID" = $4`, [nextLevel, nextRod.name, i.user.id, i.guild.id]);
                fishSuccess = true;
            } catch(e) {
                await db.query(`UPDATE user_fishing SET rodlevel = $1, currentrod = $2 WHERE userid = $3 AND guildid = $4`, [nextLevel, nextRod.name, i.user.id, i.guild.id]);
                fishSuccess = true;
            ...
            try {
                await db.query(`INSERT INTO user_fishing ("userID", "guildID", "rodLevel", "currentRod", "boatLevel", "currentBoat") VALUES ($1, $2, $3, $4, 1, 'قارب خشب')`, [i.user.id, i.guild.id, nextLevel, nextRod.name]);
                fishSuccess = true;
            } catch(e) {
                try {
                    await db.query(`INSERT INTO user_fishing (userid, guildid, rodlevel, currentrod, boatlevel, currentboat) VALUES ($1, $2, $3, $4, 1, 'قارب خشب')`, [i.user.id, i.guild.id, nextLevel, nextRod.name]);
                    ...
                    await db.query(`INSERT INTO user_fishing (userid, guildid, rodlevel, currentrod) VALUES ($1, $2, $3, $4)`, [i.user.id, i.guild.id, nextLevel, nextRod.name]);
                    fishSuccess = true;
                }
            }
        }
    ...
    if (!fishSuccess) {
        try { await db.query(`UPDATE levels SET "mora" = "mora" + $1 WHERE "user" = $2 AND "guild" = $3`, [nextRod.price, i.user.id, i.guild.id]); }
        catch(e) { await db.query(`UPDATE levels SET mora = mora + $1 WHERE userid = $2 AND guildid = $3`, [nextRod.price, i.user.id, i.guild.id]).catch(()=>{}); }
...
async function _handleBoatUpgrade(i, client, db) {
    ...
    let moraSuccess = false;
    try {
        await db.query(`UPDATE levels SET "mora" = "mora" - $1 WHERE "user" = $2 AND "guild" = $3`, [nextBoat.price, i.user.id, i.guild.id]);
        moraSuccess = true;
    } catch(e) {
        try {
            await db.query(`UPDATE levels SET mora = mora - $1 WHERE userid = $2 AND guildid = $3`, [nextBoat.price, i.user.id, i.guild.id]);
            moraSuccess = true;
        } catch(e2) {}
    }
    ...
    try {
        if (Object.keys(fishRow).length > 0) {
            try {
                await db.query(`UPDATE user_fishing SET "boatLevel" = $1, "currentBoat" = $2 WHERE "userID" = $3 AND "guildID" = $4`, [nextLevel, nextBoat.name, i.user.id, i.guild.id]);
                fishSuccess = true;
            } catch(e) {
                await db.query(`UPDATE user_fishing SET boatlevel = $1, currentboat = $2 WHERE userid = $3 AND guildid = $4`, [nextLevel, nextBoat.name, i.user.id, i.guild.id]);
                fishSuccess = true;
            ...
            try {
                await db.query(`INSERT INTO user_fishing ("userID", "guildID", "boatLevel", "currentBoat", "rodLevel", "currentRod") VALUES ($1, $2, $3, $4, 1, 'غصن شجرة')`, [i.user.id, i.guild.id, nextLevel, nextBoat.name]);
                fishSuccess = true;
            } catch(e) {
                try {
                    await db.query(`INSERT INTO user_fishing (userid, guildid, boatlevel, currentboat, rodlevel, currentrod) VALUES ($1, $2, $3, $4, 1, 'غصن شجرة')`, [i.user.id, i.guild.id, nextLevel, nextBoat.name]);
                    ...
                    await db.query(`INSERT INTO user_fishing (userid, guildid, boatlevel, currentboat) VALUES ($1, $2, $3, $4)`, [i.user.id, i.guild.id, nextLevel, nextBoat.name]);
                    fishSuccess = true;
                }
            }
        }
    ...
    if (!fishSuccess) {
        try { await db.query(`UPDATE levels SET "mora" = "mora" + $1 WHERE "user" = $2 AND "guild" = $3`, [nextBoat.price, i.user.id, i.guild.id]); }
        catch(e) { await db.query(`UPDATE levels SET mora = mora + $1 WHERE userid = $2 AND guildid = $3`, [nextBoat.price, i.user.id, i.guild.id]).catch(()=>{}); }
