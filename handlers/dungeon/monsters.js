...
const MONSTER_SKILLS = {
    ...
    "طاغية الجبال": { name: "رمي الصخور", emoji: "🪨", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.8); l.push(`🪨 **الطاغية** رمى صخرة عملاقة على **${t.name}**!`);} }},
    ...
    "قرش الرمال": { name: "كمين رملي", emoji: "🦈", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.5); t.effects.push({type:'stun',val:1,turns:1}); l.push(`🦈 **القرش** باغث **${t.name}** من تحت الرمال!`);} }},
    ...
    // ========================
    "عملاق الصقيع": { name: "نفس متجمد", emoji: "❄️", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk); t.effects.push({type:'stun',val:1,turns:1}); l.push(`❄️ **العملاق** جمد **${t.name}** بأنفاسه!`);} }},
    "الدب الفولاذي": { name: "تمزيق", emoji: "🐾", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.2); t.effects.push({type:'poison',val:Math.floor(m.atk*0.1),turns:3}); l.push(`🐾 **الدب** مزق **${t.name}** (نزيف)!`);} }}, // نستخدم poison كمحاكاة للنزيف
    ...
    "فارس الغسق": { name: "سيف الظلام", emoji: "🌑", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.6); l.push(`🌑 **فارس الغسق** وجه ضربة مشبعة بالظلام لـ **${t.name}**!`);} }},
    ...
    // ========================
    "مروض الوحوش": { name: "أمر بالهجوم", emoji: "🐕", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.4); l.push(`🐕 **المروض** أطلق وحوشه لنهش **${t.name}**!`);} }},
    "كاسر الأسود": { name: "كسر العظام", emoji: "🦴", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.3); t.effects.push({type:'weakness',val:0.2,turns:2}); l.push(`🦴 **الكاسر** حطم دفاعات **${t.name}**!`);} }},
    "الذئب الفتاك": { name: "سرعة خاطفة", emoji: "⚡", chance: 0.3, execute: (m,p,l) => { m.effects.push({type:'evasion',val:0.3,turns:2}); const t=getSmartTarget(p); if(t) applyDamageToPlayer(t, m.atk); l.push(`⚡ **الذئب** هاجم بسرعة خيالية تزيد مراوغته!`); }},
    "الأفعى العملاقة": { name: "عصر مميت", emoji: "🐍", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk); t.effects.push({type:'stun',val:1,turns:1}); l.push(`🐍 **الأفعى** عصرت **${t.name}** وشلت حركته!`);} }},
    ...
    "الوحش الضاري": { name: "تمزيق الأحشاء", emoji: "🩸", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk); t.effects.push({type:'poison',val:Math.floor(m.atk*0.15),turns:3}); l.push(`🩸 **الوحش** تسبب بجرح عميق لـ **${t.name}**!`);} }},
    "النمر الساحق": { name: "مخلب النمر", emoji: "🐅", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.5); l.push(`🐅 **النمر** سدد ضربة مخلب قوية!`);} }},
    ...
    "ساحر الظلمات": { name: "كرة الظل", emoji: "🔮", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.2); t.effects.push({type:'blind',val:0.5,turns:2}); l.push(`🔮 **الساحر** أعمى بصيرة **${t.name}**!`);} }},
    "عقرب الموت": { name: "إبرة الموت", emoji: "🦂", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk); t.effects.push({type:'poison',val:Math.floor(m.atk*0.3),turns:3}); l.push(`🦂 **العقرب** حقن سماً قاتلاً في **${t.name}**!`);} }},
    "الجزار المدرع": { name: "ساطور الجزار", emoji: "🔪", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.7); l.push(`🔪 **الجزار** هوى بساطوره على **${t.name}**!`);} }},
    "زعيم العصيان": { name: "غدر", emoji: "🗡️", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.5); l.push(`🗡️ **الزعيم** طعن **${t.name}** غدراً!`);} }},
    "الفارس الأسود": { name: "اختراق", emoji: "⚔️", chance: 0.3, execute: (m,p,l) => { const t=getSmartTarget(p); if(t){applyDamageToPlayer(t, m.atk*1.3); l.push(`⚔️ **الفارس الأسود** اخترق دفاع **${t.name}**!`);} }},
