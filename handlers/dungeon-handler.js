...
async function startDungeon(interaction, sql) {
    ...
    // 🔥🔥 معالجة زر نصب المخيم (Campfire) - يتم استدعاؤه من داخل الثريد 🔥🔥
    if (interaction.isButton() && interaction.customId === 'dungeon_campfire') {
        const gameData = sql.prepare("SELECT * FROM active_dungeons WHERE channelID = ?").get(interaction.channel.id);
        
        if (!gameData) return interaction.reply({ content: "❌ لا توجد بيانات لهذا الدانجون.", flags: [MessageFlags.Ephemeral] });
        
        const data = JSON.parse(gameData.data);
        if (interaction.user.id !== data.hostID) {
            return interaction.reply({ content: "🚫 **فقط القائد يمكنه نصب المخيم!**", flags: [MessageFlags.Ephemeral] });
        }

        // حفظ البيانات في جدول dungeon_saves (الطابق القادم)
        const nextFloor = (data.currentFloor || 0) + 1;
        
        sql.prepare("INSERT OR REPLACE INTO dungeon_saves (hostID, guildID, floor, timestamp) VALUES (?, ?, ?, ?)").run(data.hostID, interaction.guild.id, nextFloor, Date.now());

        // حذف اللعبة الحالية
        sql.prepare("DELETE FROM active_dungeons WHERE channelID = ?").run(interaction.channel.id);
        activeDungeonRequests.delete(data.hostID); 

        const campEmbed = new EmbedBuilder()
            .setTitle("⛺ تم نصب المخيم")
            .setDescription(
                `قام الفريق بنصب الخيام للراحة عند مشارف الطابق **${nextFloor}**.\n` +
                `تم حفظ تقدمك بنجاح! يمكنك العودة لاستكمال الرحلة لاحقاً.\n` +
                `*(سيتم الاحتفاظ بملف الحفظ لمدة محدودة حسب رتبتك)*`
            )
            .setColor("Green")
            .setImage("https://i.postimg.cc/KcJ6gtzV/22.jpg");

        await interaction.update({ content: "✅ تم الحفظ.", embeds: [campEmbed], components: [] });
        return; 
    }

    // --- بداية الدانجون الجديد ---
    ...
    // 🔥🔥 فحص الاستكمال (Resume Check) 🔥🔥
    let startFloor = 1;
    const save = sql.prepare("SELECT * FROM dungeon_saves WHERE hostID = ?").get(user.id);

    if (save) {
        // مدة الصلاحية حسب الرتب
        let expiryTime = 24 * 60 * 60 * 1000;
        if (interaction.member.roles.cache.has('1422160802416164885')) expiryTime = 72 * 60 * 60 * 1000;
        else if (interaction.member.roles.cache.has('1395674235002945636')) expiryTime = 35 * 60 * 60 * 1000;

        const timeLeft = expiryTime - (Date.now() - save.timestamp);

        if (timeLeft > 0) {
            // سؤال القائد: استكمال أم جديد؟
            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('resume_yes').setLabel(`استكمال (طابق ${save.floor})`).setStyle(ButtonStyle.Success).setEmoji('⛺'),
                new ButtonBuilder().setCustomId('resume_no').setLabel('بداية جديدة').setStyle(ButtonStyle.Secondary).setEmoji('⚔️')
            );
            
            // نستخدم رسالة عادية للرد لأننا سنحذفها لاحقاً
            let resumeMsg;
            if (interaction.replied || interaction.deferred) {
                resumeMsg = await interaction.followUp({
                    content: `👑 **عثرنا على مخيم سابق!**\nهل تود استكمال الرحلة من الطابق **${save.floor}**؟`,
                    components: [row],
                    fetchReply: true
                });
            } else {
                resumeMsg = await interaction.reply({
                    content: `👑 **عثرنا على مخيم سابق!**\nهل تود استكمال الرحلة من الطابق **${save.floor}**؟`,
                    components: [row],
                    fetchReply: true
                });
            ...
            try {
                const selection = await resumeMsg.awaitMessageComponent({ 
                    filter: i => i.user.id === user.id, 
                    time: 30000, 
                    componentType: ComponentType.Button 
                });
                
                if (selection.customId === 'resume_yes') {
                    startFloor = save.floor;
                    await selection.update({ content: `⛺ **سيتم استكمال الرحلة من الطابق ${startFloor}!** جاري تحضير اللوبي...`, components: [] });
                    // حذف الحفظ بعد الاستخدام (Anti-Farm)
                    sql.prepare("DELETE FROM dungeon_saves WHERE hostID = ?").run(user.id);
                } else {
                    await selection.update({ content: `⚔️ **تم اختيار بداية جديدة!** جاري تحضير اللوبي...`, components: [] });
                    sql.prepare("DELETE FROM dungeon_saves WHERE hostID = ?").run(user.id);
                }
                
                // حذف رسالة السؤال بعد قليل لتنظيف الشات
                setTimeout(() => resumeMsg.delete().catch(()=>{}), 2000);

            } catch (e) {
                return resumeMsg.edit({ content: "⏰ انتهى الوقت. ألغيت العملية.", components: [] });
            }
        } else {
            // انتهت الصلاحية
            sql.prepare("DELETE FROM dungeon_saves WHERE hostID = ?").run(user.id);
    ...
    try {
        // نمرر startFloor للوبي
        await lobbyPhase(interaction, null, selectedTheme, sql, startFloor);
    } catch (err) {
        console.error(err);
        activeDungeonRequests.delete(user.id);
        interaction.channel.send({ content: "❌ حدث خطأ أثناء بدء اللوبي." }).catch(()=>{});
    }
}

// ✅ اللوبي الأصلي (مع التعديلات الطفيفة لدعم startFloor)
async function lobbyPhase(interaction, oldMsg, theme, sql, startFloor = 1) {
    ...
    const updateEmbed = () => {
        ...
        // ✅ تم الحفاظ على شكل الإيمبد الأصلي ✅
        return new EmbedBuilder()
            .setTitle(`دانجون: ${theme.name}`) 
            .setColor(theme.color || '#2F3136') 
            .setDescription(
                `**القائد:** ${host}\n**الشروط:** لفل 5+ و 100 ${EMOJI_MORA}\n` +
                (startFloor > 1 ? `**🏕️ نقطة الانطلاق:** الطابق ${startFloor}\n` : "") + // إضافة سطر الطابق فقط إذا كان استكمال
    ...
    // إرسال اللوبي (نستخدم channel.send إذا كانت الرسالة الأصلية قد تم الرد عليها مسبقاً في مرحلة الاستكمال)
    let msg;
    if (interaction.replied || interaction.deferred) {
                ...
                try {
                    ...
                    if (fetchedMsg && fetchedMsg.embeds.length > 0) {
                        ...
                        if (reason === 'user_cancel') {
                             ...
                             const fullCooldown = 1 * 60 * 60 * 1000; // ساعة واحدة
