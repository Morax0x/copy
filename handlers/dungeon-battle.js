...
function generateBattleRow() {
    return new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder().setCustomId('class_skill').setLabel('قدرة التخصص').setEmoji('⚡').setStyle(ButtonStyle.Success),
...
// لاحظ إضافة الوسيط الأخير activeDungeonRequests لحذف الكاش عند النهاية
async function runDungeon(threadChannel, mainChannel, partyIDs, theme, sql, hostId, partyClasses, activeDungeonRequests) {
    ...
    // جلب بيانات اللاعبين مع الكلاس
    for (const id of partyIDs) {
        const m = await guild.members.fetch(id).catch(()=>null);
        if (m) {
            const cls = partyClasses.get(id) || 'Adventurer';
    ...
    for (let floor = 1; floor <= maxFloors; floor++) {
        ...
        // تصفير الدروع والمؤثرات بداية الطابق
        players.forEach(p => { 
            if (!p.isDead) { 
                p.shield = 0; 
                p.effects = []; 
        ...
        while (ongoing) {
            ...
            await new Promise(resolve => {
                ...
                collector.on('collect', async i => {
                    ...
                    try {
                        // --- معالجة زر قدرة التخصص ---
                        if (i.customId === 'class_skill') {
                             if (p.special_cooldown > 0) {
                                 await i.reply({ content: `⏳ القدرة تجهز.. انتظر ${p.special_cooldown} دور.`, ephemeral: true });
                             ...
                             switch(p.class) {
                                 case 'Leader':
                                     players.forEach(member => {
                                         if(!member.isDead) member.effects.push({ type: 'atk_buff', val: 0.3, turns: 2 });
                                     });
                                 ...
                                 case 'Priest':
                                     ...
                                     if (deadMembers.length > 0) {
                                         ...
                                         reviveTarget.hp = Math.floor(reviveTarget.maxHp * 0.2);
                                         const sacDmg = Math.floor(p.maxHp * 0.1);
                                         p.hp = Math.max(1, p.hp - sacDmg);
                                         log.push(`✨ **${p.name}** ضحى بدمه (${sacDmg}) وأحيا **${reviveTarget.name}**!`);
                                         p.special_cooldown = 7;
                                     } else {
                                         players.forEach(member => {
                                             if(!member.isDead) member.hp = Math.min(member.maxHp, member.hp + Math.floor(member.maxHp * 0.4));
                                         });
                                         log.push(`✨ **${p.name}** نشر النور المقدس وعالج الجميع (40%)!`);
                                 ...
                                 case 'Tank':
                                     ...
                                     log.push(`🛡️ **${p.name}** استفز الوحش وصلب درعه! (سيتلقى كل الضربات).`);
                                 ...
                                 case 'Mage':
                                     monster.frozen = true;
                                     log.push(`❄️ **${p.name}** جمد الوحش بسجن جليدي! (الوحش سيخسر دوره).`);
                                     p.special_cooldown = 5;
                                     skillUsed = true;
                                     break;
                                 ...
                                 default:
                                     await i.reply({ content: "ليس لديك قدرة خاصة.", ephemeral: true });
                                     break;
                             }
            ...
            if (monster.hp <= 0) {
                ...
                if (monster.frozen) {
                    ...
                    if (alivePlayers.length > 0) {
                        ...
                        if (target) {
                             target.effects.forEach(e => { if (e.type === 'def_buff') defMultiplier -= e.val; });
                        }
